...
class Program
{
    ...
    static void EqipManage()
    {
        while (true)
        {
            ...
            {
                if (num == 0) break;
                Item selectedItem = player.inven[num - 1];

                if (selectedItem is EquipmentItem equipmentItem)
                {
                    equipmentItem.eqip(player);
                }
            }
        }
    }
    ...
    public static void WriteColoredNumbers(string text, ConsoleColor numberColor, ConsoleColor textColor)
    {
